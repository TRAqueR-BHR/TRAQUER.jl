...
# These are populated by the individual api-def/*.jl files that are included below.
api_routes  = ()
mux_filters = ()
...
# Include all endpoint definition files from this directory
let
    api_def_dir = joinpath(@__DIR__, "api-def")
    for f in sort(filter(x -> endswith(x, ".jl"), readdir(api_def_dir)))
        include(joinpath(api_def_dir, f))
    end
end

...
@testset "Test KdfChildKeyCtrl.generateChildKey matches gen_child_pwd.sh" begin
    ...
    base64_script_output = readchomp(`bash $script_path $parent_key_hex $salt_hex $ref $key_format`)
    expected_base64_child_key = match(r"(?m)^Child key:\s*(.+)$", base64_script_output).captures[1]

    @test KdfChildKeyCtrl.generateChildKey(parent_key_hex, salt_hex, ref, key_format) == expected_base64_child_key

    key_format = BinaryEncoding.hex
    hex_script_output = readchomp(`bash $script_path $parent_key_hex $salt_hex $ref $key_format`)
    expected_hex_child_key = match(r"(?m)^Child key:\s*(.+)$", hex_script_output).captures[1]

    @test KdfChildKeyCtrl.generateChildKey(parent_key_hex, salt_hex, ref, key_format) == expected_hex_child_key
end
